...
extension ChessMove {
    ...
    @objc
    public func sanString() -> String {
        if self.isNullMove() {
            return "0000"
        }
        let labels = [ kPawn: "", kKnight: "N", kBishop: "B", kRook: "R", kQueen: "Q", kKing: "K" ]
        let ranks = ["a", "b", "c", "d", "e", "f", "g", "h"]
        let files = ["1", "2", "3", "4", "5", "6", "7", "8"]
        var result = ""

        result += labels[Int(movingPiece)]!
        result += ranks[Int(sourceSquare & 7)]
        result += files[Int(sourceSquare >> 3)]
        result += (capturedPiece == 0) ? "" : "x"
        result += (capturedPiece == 0) ? "" : labels[Int(capturedPiece)]!
        result += ranks[Int(destinationSquare & 7)]
        result += files[Int(destinationSquare >> 3)]

        return result
    }

    @objc
    public func uciString() -> String {
        if self.isNullMove() {
            return "0000"
        }
        let ranks = ["a", "b", "c", "d", "e", "f", "g", "h"]
